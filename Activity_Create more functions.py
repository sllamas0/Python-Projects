...
# I observed that the numbers are sorted from least to greatest instead of what was typed in. I notice that in one particular month, they jumped from 119 to 223 which shows a great change.
...
# when executed, typed the highest number of failed logins within the list.
...
# it prints current day login total for X (username) and Y (current day logins). It provides the foundation to have different usernames and login totals for almost anyone, since it is not hard coded.
...
# This version displays the current logins, average logins, and the ratio of the two. It would be different for each username put in.
...
# It includes a return instead of print, so that means the function is able to store information instead of just printing it.
