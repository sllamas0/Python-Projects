...
# I observed that it displayed the first in the list of approved_users, as well as the first in the list of approved_devices. When you replace the 0 with another index it reads the list from left to right until it hits that integer.
...
# when approved users is displayed, gesparza is added into the list. when approved devices is displayed "3rcv4w6" is displayed as well.
...
# when approved_users is displayed, the removed user is not on the list. Likewise when approved_devices is displayed, the remove device is not on the list.
...
# when the username is set to sgilmore then the output is that it prints out the statement allowing access into the system.
...
# when the username "sgilmore" the output is 2, showing the username to be the third in the list counting from zero, or second, counting from 0.
...
# When the username is sgilmore, it pritns the corresponding device id to that username. 
...
# When username is "sgilmore" and device_id is 4n482ts the output is that sgilmore is approved to access to the system. also, it shows the device id assigned to sgilmore.
...
# i observed that when the username and device id match it prints out saying they have access and what their device id is. If they use an approved username but a different device then they are assigned they get a message saying they are an approved user but that device is not assigned to them.
