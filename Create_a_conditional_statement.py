...
# When value of system is OS 2, the script prints out that no update is needed. However when assinging the value of system to "OS 1 then the system does not print out anything, suggesting that an update may be needed.
...
# When the system is set to OS 2 then the system prints that an update is not needed, but for anything else, it will print "update needed".
...
# When OS 2 is running, the script prints "no update needed. For OS 1 & OS 3, then the script prints out "update needed". If you set the system to anything else, nothing will be printed.
...
# I noticed this conditional, is more simple than the one before, which is easier to read.         
...
# When an approved user tries to login, they are met with the text "This user has access to this device". If they are not on the list, then the script responds with "This user does not have access to this device"
...
# If user logs in during organizaton's hours, then the script will print that they attempted to login during buisnesses hours. Otherwise, it will state that an attempt was made outside of organization's hours
...
# If the user tries to log in and is not amoung the list of approved users, then the script will print that the user does not have access to this device. If they are amoun the list of approved users, then it will print "This user has access to this device. When the user tries to log in outside of organization hours, then the script will print "Login attempt made outside of organization hours".
