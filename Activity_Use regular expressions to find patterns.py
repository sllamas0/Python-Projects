...
# the regular expression was just the /w+, so in combination was "r15\w+". Without the r15 the pattern isnt going to matchw ith the device IDs that start with "r15". removing the /w will not match with alphanumeric character following "r15". Without the +, pattern will match with only the first alphanumeric character that occurs after the"r15"
...
# One example of an IP address that was not extracted was "192.16874.1390.176". These IP addresses do not seem to be valid as they do not follow the IPv4 protocol. Another example was "192.168.22.115" this was a valid IP address, but it did not follow the form because of the 3rd subset.
...
# Everything gets extracted. Those with less than three in every segment, as well as those with more than three in the segment.
...
# All these IP addresses seem to include those with less than 3 digits and all are valid within the IPv4 protocol.
